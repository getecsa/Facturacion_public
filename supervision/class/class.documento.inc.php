<?php
/**
* Acciones con las Documentos (accesos, validación, etc.)
*
* PHP version 5
*
...
* @version 1.0 Oct-14
*/
class Documento extends DB_Connection
{
	private $_perfil;
	private $_perfilSel;
	private $_bandeja;
	...
	/**
	* Función para consultar los Tipos de Documentos.
	* 
	* @return array
	*/
	public function consultarTiposDocumentos()
	{
		...
		try{
			if($this->db){
		        ...
		        $sql = "SELECT id_tipo_doc,tipo_doc,suspendido 
			 			  FROM tipo_documento 
			    ...
			    if($stmt->prepare($sql)){
			    	...
			    	$stmt->bind_result($idTipoDoc,$tipoDoc,$estatus);
			    	while($stmt->fetch()){
			    		$item =array("idDoc"         => $idTipoDoc,
